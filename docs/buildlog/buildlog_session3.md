...
1. problem arises in creation of hashfunction it cannot works for objects i can create functions for int ,char,string,float that overloads each other and i also create a default function for user defined datatypes but there is an issue that shall we have to take the memory reference to return hash or we can return the sum of ascii value of the objects data members.
...
successfully build hashmap its constructor,destructor and its function like insert ,find, contains, size , isempty, clear, remove.